...
const useGameLogic = (LEVELS, showNotification, addGameStat, currentUser) => {
    ...
    const hasSavedStats = useRef(false); // Track if stats have been saved
    ...
    useEffect(() => {
        if ((gameOver || win) && !hasSavedStats.current) {
            ...
            hasSavedStats.current = true; // Mark stats as saved
            addGameStat({
                id: Date.now(),
    ...
    const resetGame = (newLevel = level) => {
        ...
        setTimeout(() => {
            ...
            setFirstClick(true);
            setLoading(false);
            hasSavedStats.current = false; // Reset stats saving flag
